...
renderSurfaceFunction <- function(
  ...
  for (i in 1:length(surfimg)) {
    surf <- as.array(surfimg[[i]])
    brain <- misc3d::contour3d(surf, level = surfval[i], alpha = alphasurf[i], draw = FALSE,
  ...
  for (i in 1:length(funcimg)) {
    if (class(mycol[[i]]) == "function") {
      ...
      mask_shell <- mask_shell - iMath(mask_shell, "ME")
      func <- as.array(funcimg[[i]]) * as.array(mask_shell)
      min_func <- min(func)
      max_func <- max(func)
      lut <- mycol[[i]](32)
      # if (!missing(baseval)) # white out all values below baseval
      #   lut[1:round((basefval - min_func) / max(basefval - min_func) * (32 - 1) + 1)] <- "white"
      func <- round((func - min_func) / max(func - min_func) * (32 - 1) + 1)
      # render suface of each level
      progress <- txtProgressBar(min = 0, max = 32, style = 3)
      for (j in 1:32) {
        tmp  <- array(0, dim = dim(funcimg[[i]]))
        tmp[func == j] <- 1
        brain <- misc3d::contour3d(tmp, level = 0, alpha = alphafunc[i], draw = FALSE,
                                   smooth = FALSE, material = "metal", depth = 0.6, color = lut[j])
        if (physical == TRUE) {
