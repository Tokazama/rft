# To do:
# very unfinished but the goal is to incorporate more formulaic short hand for many of ANTsR anlysis functions so that they are more user friendly.

(formula, iData, mask = NA, sparseness = 0.05, nvecs = 10, its = 5, cthresh = 250, statdir = NA, z = 0, smooth = 0)

ilms <- function(inmatrix, demog, outcome, mask = NA, sparseness = 0.05, 
  ...
  args <- list("--svd", paste("network[", matname, ",", mfn, 
                              ",", sparseness, ",", demog.name, "]", sep = ""), "--l1", 
               1, "-i", its, "--PClusterThresh", cthresh, "-n", nvecs, 
               "-o", outfn, "-z", z, "-s", smooth)
  ...
  if (!is.na(mask)) {
    ...
    fnl <- list.files(path = statdir, pattern = glob2rx(glb), 
                      full.names = T, recursive = T)[1:nvecs]
    fnll <- list()
  ...
  glb <- paste("spcaprojectionsView1vec.csv", sep = "")
  fnu <- list.files(path = statdir, pattern = glob2rx(glb), 
                    full.names = T, recursive = T)
  fnu <- read.csv(fnu)
