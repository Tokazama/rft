...
#' Fit images to general linear model
#' 
#' @param formula
#' @param iData
#' @param weights
...
#'
#' @export iglm
iglm <- function(formula, iData, weights = NULL, control, verbose = TRUE, ...) {
  cl <- match.call()
  z <- iFormula(formula, iData)
  ...
  for (i in seq_len(length(z$y))) {
    if (verbose)
      cat(pase("Fitting response", z$y[i], "\n"))
    out[[i]] <- iModel(X = z$X, y = z$y[i], data = iData, weights = weights, control = control, ...)
    out[[i]]@method <- c("ilm", "iwls")
    ...
    while(max(abs(ores - nres)) > sqrt(1e-8)) {
      ...
      if (n == 1) {
        W <- matrix(1, out[[i]]@dims$nimg, out[[i]]@dims$nvox)
        W[is.na((out[[i]]@iDataF@iList[[y]]@iMatrix[]))] <- 0
      }
      ...
      if (n > out[[i]]@control$mi) {
        warning('rftGlm could not converge. Maximal number of iterations exceeded.');
        break
      }
    ...
    if (control$rft) {
      smooth <- estSmooth(out[[i]]@res[], out[[i]]@iData[[out[[i]]@y]]@mask, out[[i]]@X$rdf, scaleResid = FALSE, sample = ctrl$sr, verbose = verbose)
      out[[i]]@dims$fwhm <- smooth$fwhm
      out[[i]]@dims$rpvImage <- smooth$rpvImage
      out[[i]]@dims$resels <- resels(out[[i]]@iData[[x@y]]@mask, smooth$fwhm)
    }
