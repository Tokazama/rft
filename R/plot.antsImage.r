...
plot.antsImage <- function(x, y,
  ...
  if(colorbar){
    levels <- seq(window.img[1], window.img[2], length.out=gradbar)
    ...
    rect(0, levels[-length(levels)], 1, levels[-1L],
         col = makePalette(color.colorbar, gradbar))
    axis(4)
    box()
...
#      stop("Image and overlay image sizes do not match.")
    ...
    if (color.overlay[ind] != "jet")
		colorRampAlpha <-function(..., n, alpha){
			colorfun <-colorRampPalette(c("#00007F", "blue", "#007FFF", "cyan","#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000"), interpolate = c("spline"))(n)
			paste(colorfun, sprintf("%x", ceiling(255*alpha)), sep="")
			}
    if (color.overlay[ind] == "jet") {
      # print('use jet')
		colorRampAlpha <-function(..., n, alpha){
    ...
    heatvals <- colorRampAlpha(n=nlevels, alpha=alpha)
    if (locthresh[1] > 1)
